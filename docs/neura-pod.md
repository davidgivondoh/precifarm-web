...
| **Battery management system (BMS)** | Per-cell voltage and temperature monitoring, balancing, overcurrent / over-temp / undervoltage protection. Exposes data to PreciSense. |
| **Hybrid inverter** | Pure sine wave output with built-in MPPT solar charger. See sizing table below. |
| **PreciSense telemetry board** | On-device sensor that reports voltage, current, temperature, and state of charge to the Neura platform every minute. |
...
| **Neura Pod 5** | 5 kWh LiFePO4 (Dyness / MUST) | 3 kW hybrid (Growatt or equivalent) | Precifarm Family | 7.43 kWh (4 × 550W panels) | ~4 kWh/day | ~1.2 days |
| **Neura Pod 15** | 15 kWh LiFePO4 (3 × 5 kWh stack) | 12 kW hybrid (Deye, commercial grade) | Precifarm Commercial | 44.6 kWh (24 × 550W panels) | ~28 kWh/day | ~0.7 days |
...
Customer-facing example (Family / Pod 5): subtotal KSh 288,000 + 8% premium = KSh 311,040 landed + 20% markup = KSh 373,000 selling price.

## The Neura platform — three layers

The Pod is the hardware. Neura is the software brain.

1. **PreciSense** — on-device telemetry board on every charge controller. Reports voltage, current, temperature, and state of charge every minute over a low-bandwidth uplink.
2. **Neura AI** — the platform that ingests PreciSense data, runs predictive fault detection, and routes alerts to the right field engineer with the right part already in hand.
3. **Customer surfaces** —
...
- Component cost basis (Kenya, May 2026): `Precifarm_Solar_Pricing_Financing_v2.pdf` (source of truth)
